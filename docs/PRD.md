...
- **Trip auto-assignment:** ~~incoming reservation date falls inside exactly one trip's range → auto-assign; zero or multiple matches → "needs trip" inbox.~~ Superseded by [ADR 0001](adr/0001-probabilistic-trip-assignment.md): assignment is probabilistic (AI weighs dates, destinations, geography, email content into an assignment confidence); the trip's booking window is the only hard bound.
